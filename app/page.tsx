...
export default function Studio(){
 ...
 const [scale,setScale]=useState(210),[pos,setPos]=useState(5900),[playing,setPlaying]=useState(false),[selected,setSelected]=useState<number|null>(0),[range,setRange]=useState<[number,number]>([events[0].t,events[0].end]);
 ...
 useEffect(()=>{const f=(ev:KeyboardEvent)=>{if(!isWeather||document.querySelector('dialog[open]'))return;if((ev.target as HTMLElement).closest('form,input,textarea,select,button,a,summary,[contenteditable="true"]'))return;if(ev.code==='Space'){ev.preventDefault();if(ev.repeat)return;setPos(p=>p>=END?0:p);setPlaying(p=>!p)}if(ev.code==='Enter'||ev.code==='NumpadEnter'){ev.preventDefault();if(ev.repeat)return;setPos(0);setPlaying(false);if(scroller.current)scroller.current.scrollLeft=0}if(ev.key==='Escape'){setSelected(null);setRange([0,0])}};window.addEventListener('keydown',f);return()=>window.removeEventListener('keydown',f)},[isWeather,END]);
