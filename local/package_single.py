...
with zipfile.ZipFile(out/'Track Studio Source.zip','w',zipfile.ZIP_DEFLATED) as z:
 for item in ['app','components','lib','hooks','local','public','package.json','package-lock.json','tsconfig.json','vite.local.config.ts','next.config.ts','components.json']:
  p=root/item
  for f in ([p] if p.is_file() else p.rglob('*')):
   if f.is_file() and f.suffix not in ['.wav'] and not ('audio' in f.relative_to(root).parts and f.suffix=='.json'):z.write(f,'track-studio/'+str(f.relative_to(root)))
