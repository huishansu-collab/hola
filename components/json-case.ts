...
export type Checkpoint={t:number;end:number;name:string;title:string;note:string;quote?:string;overlap?:[number,number]};
...
export function createJsonCase(data:RawCase,timeline:RawTimeline,checkpoints:Checkpoint[],expressions:Scenario['expressions']=[]):Scenario {
 ...
 const tracks:Scenario['tracks']=timeline.tracks.map(t=>({name:t.name,en:subtitles[t.id],color:colors[t.id],clips:t.clips.map(c=>({a:c.start_at_ms,b:c.end_at_ms,label:c.label,sub:c.sub,lane:c.lane??0,...(c.audio_key?{audioKey:c.audio_key,wave:true}:{})} as Clip))}));
 const events=checkpoints.map((c,id)=>({...c,id,quote:c.quote??'',tag:c.overlap?'用户打断':'交互节点',plan:'',heard:c.quote??'',drop:'',actions:[c.note],tool:'模拟 Case',overlap:c.overlap??null}));
