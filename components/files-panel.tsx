import {createCaseId} from '../lib/case-package/id';
...
export function FilesPanel({active,onSelect,getScenario,onInspect,onImport,importedFiles}:{onImport:(file:File)=>Promise<CaseFile>;importedFiles:CaseFile[];getScenario:(id:string)=>Scenario|undefined;onInspect:()=>void;active:string;onSelect:(file:CaseFile)=>void}){
 ...
 useEffect(()=>{
  try{
   ...
   for(const file of importedFiles){let exists=false;directories=directories.map(f=>({...f,cases:f.cases.map(c=>{if(c.id!==file.id)return c;exists=true;return {...c,name:file.name}})}));if(!exists)directories[0].cases.push(file)}
 ...
 return <><div className="files-heading"><span>Files</span><div className="files-header-actions"><input ref={picker} type="file" accept=".zip,.json,application/zip,application/json" hidden aria-label="选择 Case 包" onChange={e=>{const file=e.target.files?.[0];if(file)void importFile(file)}}/><button disabled={importing} aria-busy={importing} title="导入 Case 包" aria-label="导入 Case 包" onClick={()=>picker.current?.click()}>{importing?<span className="synthesis-spinner"/>:<Upload size={14}/>}</button><button title="新建 Case" aria-label="新建 Case" onClick={()=>{setCreating('case');setName('')}}><NewCase size={14}/></button><button title="新建文件夹" aria-label="新建文件夹" onClick={()=>{setCreating('folder');setName('')}}><Plus size={14}/></button></div></div>
