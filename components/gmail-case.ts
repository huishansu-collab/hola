...
export function createGmailCase():Scenario {
 ...
 const tracks=timeline.tracks.map(t=>({name:t.name,en:'',color:colors[t.id],clips:t.clips.map(c=>({a:c.start_at_ms,b:c.end_at_ms,label:c.label,sub:'sub' in c?c.sub:undefined,audioKey:'audio_key' in c?c.audio_key:undefined,wave:c.kind==='speech',fadeMs:'fade_ms' in c?c.fade_ms:undefined}))}));
 const points=[{t:4000,end:4400,name:'检查绑定',title:'读取邮箱绑定记录',note:'未绑定邮箱，先发现可用连接能力。'},{t:11080,end:11780,name:'确认 Gmail',title:'用户打断并选择 Gmail',note:'用户同意配置，邮箱尚未连接。'},{t:12400,end:12800,name:'配置卡片',title:'展示 Gmail 配置卡片',note:'等待用户按卡片指引完成配置后，再继续未读邮件摘要任务。'}];
