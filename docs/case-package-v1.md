...
音频源 v1 使用 48 kHz、16 bit、单声道或双声道 PCM WAV；双声道源在裁切时取均值。没有隐式重采样或变速。有音频的 Case 最长 600 秒；`planned` 包没有音频，允许跨小时的时程（地理围栏提醒、重型任务回访这类），上限 6 小时。所有文件引用必须位于包目录内，CLI 拒绝路径越界和指向目录外的符号链接。
