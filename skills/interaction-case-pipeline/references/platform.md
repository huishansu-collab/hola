...
完整ZIP须在导入后保留含参数的工具标题。源包依赖播放器生成标题时，检查实际使用的播放器；旧播放器仍显示空括号时，交付带完整标题的snapshot/1快照及source/源包，不只让用户换播放器。标题较长可截断显示，但详情必须可读完整变量名。

世界信号使用独立金色，与其他六轨区分。全局检查内置Case、导入Case和快照的轨道头、片段及详情涉及的配色，不仅修当前Case，也不能继续与工具轨道共用颜色。
